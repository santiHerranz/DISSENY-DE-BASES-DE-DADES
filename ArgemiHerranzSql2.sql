...
-- Primer el director del departament
...
WHERE constraint_type = 'P' and table_name = 'DEPARTAMENT';

ALTER TABLE departament 
RENAME CONSTRAINT pk_departament to pk;
...
/*
7. Comprovar quines són les restriccions definides a la taula Classe. Una d’elles no és necessària ja que és redundant. 
Detecta-la i elimina-la. Executa alguna operació de tipus DML per a demostrar que encara que aquesta restricció s’ha eliminat, es segueix satisfent.
*/
...
ALTER TABLE classe MODIFY codi NULL ;

INSERT INTO classe (codi) VALUES ('0');
...
--NO ES POSIBLE !!!
--La restricció Check no permet subconsultes
...
ALTER TABLE professor
    MODIFY departament varchar2(3) NULL;
...
-- ERROR al esborrar el departament perque hi ha professors que treballen al departament que no es poden esborrar per que son directors de altres profesors
    
ALTER TABLE professor
    DROP CONSTRAINT fk_professor_director;

ALTER TABLE professor
    ADD CONSTRAINT fk_professor_director FOREIGN KEY (director) REFERENCES professor(codi) ON DELETE CASCADE;

ALTER TABLE docencia
    DROP CONSTRAINT fk_docencia_professor;
    
ALTER TABLE docencia
    ADD CONSTRAINT fk_docencia_professor FOREIGN KEY (professor) REFERENCES professor(codi) ON DELETE CASCADE;    
...
INSERT INTO Departament (codi) VALUES ('004');
INSERT INTO Departament (codi) VALUES ('005');
INSERT INTO Departament (codi) VALUES ('006');
...
ALTER TABLE departament
  MODIFY nom NOT NULL NOVALIDATE;
...
17. A la taula professor insereix les següents files, en aquest ordre:
  codi Cognom1 Cognom2 Nom Actiu Categoria Dedicacio departament Director
  4 Marti Gomez Pere S Titular TC 001 6
  5 Cotet Jull Albert S Titular TC 001 6
  6 Adell Carpi Xavier S Director TC 002 NULL
...
-- ERROR 
/*
Si, que no existeix la clau forana al inserir els dos primers registres.

Faria que la constraint de clau forana per la columna director sigui DEFERRED
*/
